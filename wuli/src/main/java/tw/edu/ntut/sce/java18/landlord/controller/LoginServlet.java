...
public class LoginServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    // 無
    // 3. 檢查使用者輸入資料
    // 如果 mail 欄位為空白，放一個錯誤訊息到 errorMsgMap 之內
    ...
    if (rm != null) {
      cookieUser = new Cookie("mail", mail);
    ...
    // 將LandlordInfoService類別new為物件，存放物件參考的變數為 loginService
    ...
    try {
      // 呼叫 landlordInfoService物件的 checkIDPassword()，傳入mail與password兩個參數
      landlordInfo = landlordInfoService.checkIdPassword(mail, password);
      if (landlordInfo != null) {
        // OK, 登入成功, 將landlordInfo物件放入Session範圍內，識別字串為"WuliLoginOK"
        session.setAttribute("LoginOK", landlordInfo);
      } else {
        // NG, 登入失敗, mail與密碼的組合錯誤，放相關的錯誤訊息到 errorMsgMap 之內
